...
export function CanvasOverlay({
  ...
  const drawPolygonWithText = (
    ...
    if (textToShow && textToShow.trim()) {
      ...
      const fontSize = Math.max(
        12,
        Math.min(
          (bubble.font_size || 14) * Math.min(scaleX, scaleY),
          maxHeight / 4
        )
      );
  ...
  const handleCanvasMouseDown = (
    ...
    if (selectedBubbleId !== null) {
      const selectedBubble = speechBubbles.find(
        (b) => b.bubble_id === selectedBubbleId
      );
      if (selectedBubble && selectedBubble.mask_coordinates) {
        for (let i = 0; i < selectedBubble.mask_coordinates.length; i++) {
          ...
          const distance = Math.sqrt(
            Math.pow(imageX - vx, 2) + Math.pow(imageY - vy, 2)
          );
    ...
    for (const bubble of speechBubbles) {
      if (bubble.mask_coordinates && bubble.mask_coordinates.length > 0) {
        if (isPointInPolygon([imageX, imageY], bubble.mask_coordinates)) {
          ...
          const centerX =
            bubble.mask_coordinates.reduce((sum, point) => sum + point[0], 0) /
            bubble.mask_coordinates.length;
          const centerY =
            bubble.mask_coordinates.reduce((sum, point) => sum + point[1], 0) /
            bubble.mask_coordinates.length;
  ...
  const handleCanvasMouseMove = (
    ...
    if (isDraggingVertex && draggedBubble && draggingVertexIndex !== null) {
      ...
      newMaskCoordinates[draggingVertexIndex] = [
        initialVertexPos.x + deltaImageX,
        initialVertexPos.y + deltaImageY,
      ];
    ...
    const newMaskCoordinates = bubble.mask_coordinates.map((point) => [
      point[0] + offsetX,
      point[1] + offsetY,
    ]);
  ...
  const handleCanvasMouseUp = () => {
    if (isDraggingVertex) {
      // Vertex drag finished, save the new shape
    ...
    <div className="relative inline-block">
      ...
      {!isAddingBubble && !editingBubble && (
        <div className="absolute bottom-2 left-2 bg-black bg-opacity-75 text-white px-2 py-1 rounded text-sm instruction-overlay">
          Double-click speech bubble to edit text • Double-click empty area to
          add bubble • Drag bubbles to move
        </div>
      )}
    </div>
  );
