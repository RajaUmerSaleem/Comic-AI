...
export default function EditorPage() {
  ...
  const [fonts, setFonts] = useState<
    Array<{ id: number; name: string; file_url?: string }>
  >([]);
  ...
  const handleBubbleTextUpdate = async (
    ...
    try {
      // Update text
      await apiRequest(
        `/v1/pages/${selectedPageId}/bubble/${bubbleId}/text?text=${encodeURIComponent(
          text
        )}`,
        { method: "PUT" },
        token!
